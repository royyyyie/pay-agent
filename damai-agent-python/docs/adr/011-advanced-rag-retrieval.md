...
2. 语义索引使用 `semantic_text`，由显式版本化的 inference endpoint 自动分块和生成向量。中文默认采用 `sentence/max_chunk_size=200/sentence_overlap=1`，并在 Mapping 中固化；中文基线使用 `.multilingual-e5-small-elasticsearch`。更换模型或分块策略必须创建新具体索引并重新执行 Eval，不在活动索引上静默替换。
...
9. 语义索引发布采用两阶段协议：`stage` 验证端点、自动分块、Embedding、隐藏向量块及语义查询，但不切换别名；`promote` 只接受对同一具体索引生成且在有效期内的质量、负载和真实费用报告。
...
- Elasticsearch Search 不提供统一的跨推理供应商账单金额；费用门禁必须引用同一测试窗口的 Elastic Billing 或 Provider Usage 导出，字符数估算只能用于容量规划，不能作为生产验收证据。
