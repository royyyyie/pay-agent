...
当前队列是有界的“准入提示”，不是独立 Worker；客户端遗失正文时，Redis token 无法重建请求。PostgreSQL 连接按操作建立，尚需按部署并发量评估连接池/PgBouncer 容量；SSE 重连目前轮询数据库，需监控查询量。事件可能含用户输入和 Tool 数据，生产应实施保留期清理，清理后旧 `Last-Event-ID` 不再保证可重放。强杀进程、多副本与真实 Java 故障注入须在隔离环境验收，不得在云端业务命名空间直接实施。
