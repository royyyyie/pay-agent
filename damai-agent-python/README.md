...
阶段 2 已建立 Checkpoint 安全契约，并提供可选的 PostgreSQL 活动 Checkpoint Repository。迁移脚本位于 `migrations/001_agent_active_checkpoint.sql`，需由有权限的迁移账号预先执行；使用该 Repository 时额外安装 `postgres` 依赖。现有 Agent 运行时仍使用进程内会话，尚未接入 PostgreSQL/Redis 自动恢复，不能据此认定已有崩溃恢复或多副本串行能力。Checkpoint 可能包含用户输入及 Tool 参数/结果，生产使用前须完成数据库账号隔离、传输与静态加密、保留期和备份设计。详见[阶段 2 检查清单](docs/phase-2-checklist.md)。
